...
@section('title','Fotografi Mahasiswa')
...
@section('content')
 ...
 <ul class="nav nav-pills nav-justified mb-3">
    ...
    <a class="nav-link text-white" href="/voting-foto/pelajar">Pelajar</a>
  </ul>
  </div>
                    ...
                    @foreach ($result as $d)
						<div class="card mr-2 mt-2 ml-2 mb-2 col-xl-3 col-lg-4 col-md-4 col-sm-12 col-12 flex" style="width: 18rem; max-height: 30rem;">
							<a href="#">
								<img class="card-img-top" src="{{ url(asset('assets/foto')) }}/{{ $d->image_path }}"  style="max-height: 18rem; ">
									...
									<div class="text-center">
                                        <form class="myform" action="/voting/foto/mahasiswa/{{ $d->id_foto }}">
                                            @csfr
                                            <input name="_id{{ $d->id_foto }}" value="{{ $d->id_foto }}"
                                            <button type="submit" class="vote-btn" data-default-text="Voting Sekarang"
                          ...
                          <h5 class="card-title text-center">{{ $d->judul }}</h5>
								<div class="text-center" >
								    @if(Session::has('email'))
    								    ...
    								    @endforeach
    								    @if(!$hasvote)
                                    <form  action="/voting/foto/mahasiswa/{{ $d->id_foto }}" enctype="multipart/form-data" method="post">
                                        @csrf
